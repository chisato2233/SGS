...
## #33 — 2026-07-21 — 连续完成成熟规则、武将技能与 AI 路线

**原话：**
> 我希望你按照这个计划，重构所有三国杀流程，不要写测试开发代码，不要测试留着我来做，完成后更改文档状态，提交并pushgit，开始吧

**要点拆解：**
- 连续实施 0017 的 M1—M5，不把长期路线停留在文档或空接口。
- 重构标准流程、武将技能和 AI 策略，保持服务器权威、强类型规则、确定性与可扩展性。
- 不新增或运行测试代码，玩法验证由用户完成；实现完成后更新计划状态。
- 最终检查变更、提交并推送 Git。

**关联计划：** `0017-mature-rules-skills-ai-roadmap.md` 及其 M1—M5 子计划。

---

## #32 — 2026-07-20 — 服务器权威的摸牌、出牌与弃牌演出

**原话：**
> 读取目前的项目代码情况，目前还没有一个完整的摸牌动画，出牌动画和弃牌动画，你可以参考NoName相应的代码，在unreal中实现他们的效果
>
> PLEASE IMPLEMENT THIS PLAN:
> # 0011-M5：服务器权威的摸牌、出牌与弃牌演出

**要点拆解：**
- 参考 NoName 的 `$draw`、`$gain2`、`$throw` 与 `useCard/respond`，实现全桌开局/回合/奖励摸牌、出牌/响应、自动弃牌与死亡清牌演出。
- 规则立即结算，动画只消费服务器有序表现事件；公共/私有快照必须确保对手摸牌不泄漏牌面。
- 增加牌堆、中央出牌区、弃牌堆、目标连线和命中测试透明的单 Active Timer Motion Layer。
- 重复、乱序、纪元切换、事件窗口缺口、队列溢出和 HUD 销毁必须安全快进或清理。

**关联计划：** `0011-M5-authoritative-card-motion.md`。

## #31 — 2026-07-20 — AI Pass 原因与可见思考时间

**原话：**
> 目前怎么有好多AI什么都不做就跳过回合啊？以及即使你什么都不做也要思考一段时间吧？

**要点拆解：**
- 查明当前 AI 大量 Pass 是合法候选与基础牌评分共同作用，而非决策流程卡死。
- AI 即使选择 Pass 或不响应，也不应在同一帧瞬间推进；增加不影响规则确定性的可配置思考延迟。
- 不伪造动作，不让 AI 为了表现而使用零收益牌。

**关联计划：** `0017-M1-information-safe-ai-evaluation.md`。
...
**原话：**
> 接下来我们开始最小可玩demo集中开发，请注意我需要你在这一轮开发中遵循敏捷原则，将下面的话当作重要的本次开发的核心原则：1. 不为不会出现的错误处理，不增加额外代码，2. 最大程度复用现有工具库，3. 像一个外科手术医生一样精准的判断我们的目标需要哪些最小代码实现，剔除冗余。4. 不要写测试代码，不要写log代码，这些留着我手动测试 ，最小程度节省输出token
>
> PLEASE IMPLEMENT THIS PLAN:
> # 0015：最小可玩八人身份 Demo

**要点拆解：**
- 固定实现 `1 真人 + 7 AI`、`1 主公 + 2 忠臣 + 4 反贼 + 1 内奸`，覆盖随机开局、基础回合、身份胜负与终局展示。
- 补齐杀、闪、桃、酒、连续出牌、自动弃牌、死亡奖惩及身份感知的简单 AI。
- 最大程度复用现有规则工具与 UI；不增加未发生错误的防御分支、额外框架、测试代码或日志代码。
- 本轮只做最小 Demo 范围，自动化测试留空，由用户手工完成玩法验收。

**关联计划：** `0015-minimal-playable-identity-demo.md`。

---

## #27 — 2026-07-12 — 移除游戏内调试日志并提供 SGS 编辑器日志工具

**原话：**
> 优化开发阶段这里的日志窗口。。。其实可以不用这个或者开发一种编辑器工具用于SGS的专属日志的显示而不需要渲染在游戏中，，直接实现！

**要点拆解：**
- 删除牌桌中央的开发调试文本，不再通过 Table Snapshot 向游戏 UI 复制最后一条命令日志。
- 新增仅随 Editor Target 加载的 `SGSEditor` 模块，复用 Unreal Message Log 展示 `LogSGS*` 分类。
- Runtime / Shipping 不依赖编辑器日志 UI，权威命令审计与结构化 `UE_LOG` 继续保留。

**关联计划：** `0011-native-code-first-ui.md`。

---

## #26 — 2026-07-12 — 参考 NoName 构建统一响应 UI

**原话：**
> 查找NoName响应条的实现形式，构建一套响应UI系统用于询问响应杀闪，询问响应无懈可击，询问打出杀闪以响应万箭齐发啊，询问使用技能这些交互！

**要点拆解：**
- 参考 NoName 的统一选择事件、技能控制组和确认/取消响应条，而不是为每张牌单独制作 UI。
- 支持结构化提示、合法手牌、目标、可用技能以及确认/不响应。
- 当前真实接入杀→闪与濒死→桃；无懈、AOE 杀/闪和技能由同一请求接口扩展，不在 UI 中写死规则结算。

**关联计划：** `0011-M4-unified-response-ui.md`。

---

## #25 — 2026-07-11 — 手牌宽度与高性能物理感交互

**原话：**
> 牌有点窄，加宽一下，下一步我想要专注于手牌的交互性能，我需要能够：1. 足够跟手 2. 与其他牌有足够的动画互动，比如拖拽有自然的更换位置动画啊，带有阻尼感的随鼠标移动并在松开鼠标后一个曲线动画的回归原位置啊，，，之类的，，，首先基于我的需求，有没有现成的unreal中的解决方案。。。避免重复造轮子！

**要点拆解：**
- 手牌在现有高度下适度加宽。
- 下一阶段重点是低延迟拖拽、邻牌让位/换位动画、阻尼跟随与松手回位。
- 优先复用 Unreal 的 Slate DragDrop、RenderTransform、CurveSequence、SpringDamper、Invalidation 与性能分析能力，不开发平行 UI runtime。

**关联计划：** `0011-native-code-first-ui.md`；正式实施前应拆出独立手牌交互里程碑。

---

## #24 — 2026-07-11 — 放大武将牌与手牌并修复鼠标操作

**原话：**
> 有点小bug，首先就是武将牌不够大，增大一些，最好能自然一点，包括手牌也是，其次是现在好像鼠标没有办法操作对吗？

**要点拆解：**
- 武将牌和手牌按 viewport 响应式放大，本地武将保持更明确的视觉层级。
- 手牌居中自然展开，并同步调整手牌栏与控制栏，避免只放大图片造成区域挤压。
- 修复高 ZOrder Toast 展示层遮挡 Table 鼠标命中，并明确配置 LocalPlayer 鼠标输入模式。
