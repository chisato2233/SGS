...
- 2026-07-11：修复首轮人工体验反馈。对手武将基准尺寸提升至 `168x232`，本地武将再放大 10%；手牌恢复 `94x132` 牌面比例并在可用宽度内居中展开，布局缩放上限由 `1.0` 提升到 `1.25`，使 1600x900/1920x1080 大窗口继续自然增长。鼠标不可操作的根因是 ZOrder 20 的全视口 Toast 层参与命中测试，现已设为 `HitTestInvisible`；PlayerController 同时显式启用 click/hover、保留捕获时光标并把焦点交给 Table HUD。Development Editor 编译通过，等待用户 PIE 主观尺寸与真实点击复验。
- 2026-07-11：根据第二轮视觉反馈将手牌宽度由 `94` 调整为 `104`、高度保持 `132`。下一阶段拟单独规划高性能手牌交互，优先复用 Slate DragDrop、RenderTransform、CurveSequence、Core SpringDamper 与 Invalidation，不在 M1 Widget 中直接堆叠拖拽和动画状态。
