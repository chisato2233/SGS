...
void FSGSCommandRouter::RegisterCommandType(TSharedRef<ISGSCommandType> CommandType)
{
	...
	for (int32 Index = TypeSpecs.Num() - 1; Index >= 0; --Index)
	{
		...
		{
			ensureMsgf(false, TEXT("Replacing SGS command type for type %s."), *Type.ToString());
...
FSGSStatus FSGSCommandRouter::SubmitCommand(const FSGSCommand& Command, const FSGSCommandExecutionContext& Context)
{
	...
	if (FSGSStatus Status = TypeSpec->Validate(Command, Context); Status.HasError())
	{
		RecordLifecycle(Command, SGSCommandLifecycle::Rejected(), false, Status.GetError());
		return Status;
	}
	...
	if (FSGSStatus Status = TypeSpec->Execute(Command, Context); Status.HasError())
	{
	...
	UE_LOG(LogSGSTurn, Verbose, TEXT("Executed SGS command: %s PayloadSummary=%s"), *Command.ToLogString(), *TypeSpec->FormatPayloadSummary(Command));
	return MakeValue();
}
...
FSGSStatus FSGSCommandRouter::ValidateCommon(const FSGSCommand& Command, const FSGSCommandExecutionContext& Context) const
{
	...
	const ISGSCommandType* TypeSpec = Command.Type.IsValid() ? FindTypeSpec(Command.Type) : nullptr;
	if (TypeSpec == nullptr)
	{
		return MakeError(FSGSError::Make(
			FName(TEXT("SGS.Command.UnsupportedType")),
			FString::Printf(TEXT("No SGS command type registered for type %s."), *Command.Type.ToString())));
	}
	...
	if (ExpectedPayloadStruct == nullptr)
	{
		return MakeError(FSGSError::Make(
			...
			FString::Printf(TEXT("Command type %s does not declare a payload struct."), *Command.Type.ToString())));
	}
	...
	if (ActualPayloadStruct == nullptr)
	{
		return MakeError(FSGSError::Make(
			...
			FString::Printf(TEXT("Command type %s requires payload %s."), *Command.Type.ToString(), *ExpectedPayloadStruct->GetName())));
	}
...
const ISGSCommandType* FSGSCommandRouter::FindTypeSpec(FGameplayTag Type) const
{
	for (const TSharedRef<ISGSCommandType>& TypeSpec : TypeSpecs)
	{
...
FString FSGSCommandRouter::FormatPayloadForLog(const FSGSCommand& Command) const
{
	const ISGSCommandType* TypeSpec = Command.Type.IsValid() ? FindTypeSpec(Command.Type) : nullptr;
	if (TypeSpec != nullptr)
	{
	...
	return Command.HasPayload()
		? FString::Printf(TEXT("UnregisteredPayload=%s"), *Command.GetPayloadTypeName())
		: TEXT("NoPayload");
}
