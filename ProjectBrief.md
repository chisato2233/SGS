...
- 规则层已有核心地基：Command、RandomAudit、IndexedStore/TargetQuery、Timing/ActiveEffect、EffectPipeline、ReplayLog、`USGSGameContext`、`USGSGameDriver`。Plan 0005 已补齐杀 / 闪 / 桃与濒死求桃的最小可玩规则纵切。代码细节不要在本文件展开，使用 graphify 查询。
...
- 当前 `In Progress` 的实现 Plan：`0011-M1`。
...
- 后续按核心逻辑路线继续推进：判定 / 延时锦囊、即时锦囊、装备、武将技、AI、网络。
