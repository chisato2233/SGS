...
- **进行中**：
  - [~] Plan 0003：权威对局骨架（服务器侧回合阶段机闭环 + 决策代理 + 事件总线 + 占位 AI + GameMode）。复制/RPC 拆到 0003N（需可编译 UE 环境）。
- **下一步（按 Plan 0002 路线图）**：
  - [ ] 0003N 网络层（复制 + RPC 决策通道）→ 0004 牌堆手牌 → 0005 杀/闪/桃 → 0006 判定/延时锦囊 → 0007 即时锦囊 → 0008 装备 → 0009 武将技 → 0010 AI 代理 → 0011 UI → 0012 联机打磨。
